...
export PATH="/mnt/j/msys64/mingw64/bin:$PATH"

if service postgresql status | grep -q down
  then
...
if [ $TODAY != $LAST ]
  then
    ...
    NODE_REMOTE="$(nvm ls-remote | tail -1 | egrep -o -m 1 v[[:digit:]]+.[[:digit:]]+.[[:digit:]]+)"
    echo $TODAY > .nvmcheck
    if [ $NODE_LOCAL != $NODE_REMOTE ]
      then
        echo "A new version of Node is available ($NODE_REMOTE vs $NODE_LOCAL)"
        read -p "Install? [y/n] " yesno
        if [ $yesno == "y" ]
          then
            nvm install node
            nvm reinstall-packages $NODE_LOCAL
            nvm uninstall $NODE_LOCAL
        fi
    fi
